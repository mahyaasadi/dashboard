	...
	const columns = [
        {
			...
			name: 'Patient',			
			sortable: true,
			cell: row => <Media ><Media.Body className="d-flex"><Link to="/admin/profile" className="avatar avatar-sm me-2 user-dt" ><img src={row.img_url} className="avatar avatar-img" /></Link><div className="text-secondary"><span className='user-name'>{row.Patient}</span><br/>
      ...
      <span className="d-block tab-subtext">{row.Department}</span></div></Media.Body></Media>,
		...
		{
			name: 'Disease',
			selector: row=><Media><span className='disease-name'>{row.Disease}</span></Media>,
			sortable: true,	
			width:"250px",			
		},
        {
